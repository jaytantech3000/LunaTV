...
> **2026-06-09 方向修正**
>
> 本文早期版本把桌面版收敛成“本地数据面 + 远端帐号/用户数据同步 + 裁剪后台”的窄目标，这个判断已经失效。
>
> 当前主线改为：
>
> - 桌面版视为独立部署形态，尽量复刻 Web 版能力
> - 本地多用户 / 本地管理后台进入主线，不再延后到桌面版之后
> - 多端帐号同步（桌面 / 手机 / Web）延后，先记录，不作为当前桌面版边界
>
> 因此，本文中凡是“桌面 v1 不迁移后台 / 不做多用户 / 只同步帐号与用户数据”的表述，均以下面的修正为准：
>
> - 第一优先级仍然是保住本地搜索、播放、直播、下载和代理链路
> - 但桌面管理能力不再按“隐藏或降级”处理，而是按“复用现有 Web 管理面并在本地补齐后端能力”推进
> - `profile_sync.api_base_url` 仅保留为后续扩展能力，不再定义桌面版范围

> **目标顺序** · 桌面版 -> 手机版 -> TV 版  
> **当前结论** · 先拆服务依赖，再上平台壳  
> **当前推荐策略** · 本地数据面 + 本地管理能力 + 复用 Web 前端 + 渐进补齐桌面本地服务接口
...
- 桌面 v1 保持搜索、详情、播放、下载和媒体代理本地运行
- 桌面 v1 仅同步帐号认证与用户数据（播放记录 / 收藏 / 搜索历史 / 跳过片头片尾）
- 桌面前端不直连远端 Web 后端；帐号与用户数据通过本地 Rust 服务代理转发
...
- 本地搜索 / 详情 / 播放 / 直播 / 下载 / 媒体代理
- 本地配置文件
- 远端帐号认证同步
- 远端播放记录 / 收藏 / 搜索历史 / 跳过片头片尾同步

补充约束：

- 桌面版只同步帐号和用户数据，不同步媒体缓存、下载任务和资源站配置
- 桌面前端始终只请求本地 loopback service，由本地服务决定哪些接口代理到远端
...
- 本地下载
- 帐号登录同步
- 播放记录 / 收藏 / 搜索历史 / 跳过片头片尾同步
- 本地配置文件读取

### 应做

- 豆瓣 / Bangumi 只保留能直接复用的只读能力
- 本地访问密码作为未启用云端同步时的回退认证
- 基础错误恢复和日志导出
