...
- 除非需求中明文提及 `main`，否则不要让 `main` 跟随其他分支一起改动。
- 本仓库所说的 `web` 分支，指的是 `nova` 和 `luna` 两个分支。
- `luna` 是 `web` 正式分支，用于正式发布、稳定版本维护和生产部署。
- `nova` 是 `web` 测试分支，用于新功能验证、预发布测试和风险隔离。
- `web` 相关功能默认先进入 `nova` 验证，确认稳定后再同步到 `luna`。
- 日常开发、修复、发布相关工作请放在 `desktop`、`nova`、`luna` 或其他非 `main` 分支进行。
...
### Desktop diagnostics upload (optional)

When the desktop app exports a diagnostics report, it can also POST a copy to `profile_sync.api_base_url + /api/desktop/diagnostics/upload`.

Enable the relay on the web site only if you want this behavior:

- `DESKTOP_DIAGNOSTICS_UPLOAD_ENABLED=true`
- `DESKTOP_DIAGNOSTICS_GITHUB_TOKEN=<GitHub token with issue write access>`
...
- GitHub credentials stay on the web server; they are not shipped inside the desktop client.
- If `profile_sync.api_base_url` is not configured, the desktop app still exports the local `.txt` log and skips auto-upload.
